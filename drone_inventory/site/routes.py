...
@site.route("/")
def home():
    print("Heres the message")
    return render_template('index.html')

@site.route('/profile', methods = ['GET', 'POST'])
@login_required
def profile():
    my_drone = DroneForm()  #data coming back from my drone
    ...
    try:
        if request.method == "POST" and my_drone.validate_on_submit():
            ...
            if my_drone.dad_joke.data:
                random_joke = drone.dad_joke.data
            else:
        ...
        raise Exception('Drone not created please check your form again')
